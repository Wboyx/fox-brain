...
def collect_context(cfg, level="normal"):
    ...
    add(
        f"لاگ سرویس {cfg['log_service']}",
        f"journalctl -u {cfg['log_service']} --since '30 min ago' --no-pager 2>/dev/null | tail -n {log_lines}",
        timeout=30,
    )
...
مرحله بعد:
فقط یک دستور تشخیصی خواندنی پیشنهاد بده تا فرضیه را تأیید یا رد کند.

نکات مهم:

- تو دسترسی اجرا نداری. فقط تحلیل می‌کنی و پیشنهاد می‌دهی.
- هرگز دستور مخرب پیشنهاد نده.
