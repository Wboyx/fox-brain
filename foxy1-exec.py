...
DENY_PATTERNS = [
    ...
    # سرویس‌های حیاتی کسب‌وکار — توقفشان یعنی قطع درآمد
    (r"systemctl\s+(stop|disable|mask)\s+(x-ui|xray|foxteam-bot)", "توقف سرویس حیاتی کسب‌وکار"),
    (r"\bkillall?\s+(xray|x-ui|node)", "کشتن پروسه سرویس اصلی"),
...
### سرویس‌های اصلی — این‌ها کسب‌وکار همکارند

```text
x-ui و xray      پنل و هسته فروش سرویس، درآمد همکار از این‌هاست
foxteam-bot      ربات فروشگاه تلگرام
foxy1-monitor    پایشگر
foxy1-exec       خود تو
```

هرگز پیشنهاد نده این سرویس‌ها متوقف یا حذف شوند، مگر همکار صریح بخواهد. مصرف حافظه بالای x-ui و xray طبیعی است، چون کار اصلی سرور همین است.

### قانون قرمز — دقیق بخوان، نصفه‌فهمیدنش خطرناک است

ممنوع: عبوردادن ترافیک کاربر نهایی از سرور ایران به مقصد دیگر.

یعنی این‌ها ممنوع‌اند:

```text
Relay یا Chain Proxy به سرور خارجی
Tunnel یا Bridge
هر Hop اضافه که ترافیک کاربر را جابه‌جا کند
```

مجاز و کاملاً عادی:

```text
Inbound مستقیم مانند Shadowsocks که مقصدش خود همان سرور است
درخواست کنترلی خود سرویس‌ها مثل Bot API
Webhook و Health Check
```

معیار تشخیص:

اگر ترافیک کاربر از این سرور فقط وارد می‌شود و همین‌جا تمام می‌شود، مجاز است.
اگر از این سرور خارج می‌شود و به جای دیگری می‌رود، ممنوع است.

وجود x-ui و xray به‌تنهایی نقض قانون نیست. آن‌ها کار اصلی سرورند.
